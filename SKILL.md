...
8. For beginner validation, use `book/references/beginner-reader-kit.md` together with `book/references/beginner-validation-protocol.md`; run `scripts/prepare-beginner-pilot.py` through `init`, consent-gated `new-attempt`, and one-way `finalize`; freeze the cohort with `book/references/beginner-pilot-cohort-manifest.schema.json`, record every ordered attempt with `book/references/beginner-pilot-record.schema.json`, and pass only the final manifest to `scripts/score-beginner-pilot.py`. A generated draft is intentionally invalid and is never participant evidence.
...
12. Build `scripts/build-external-review-packet.py` from a clean candidate when issuing work orders. The ZIP must include the complete frozen beginner-pilot preparation and scoring runtime alongside its protocols and schemas. Treat it as a deterministic handoff aid, never as evidence that a gate passed.
...
For EPUB, run the packaging script, EPUBCheck, and DAISY Ace when available. Inspect metadata, cover, table of contents, internal anchors, external links, font resizing, dark mode, and reading order in at least one standards-based reader. For PDF, run the pinned veraPDF gate in addition to metadata, tagging, geometry, text, and rendered-page checks. Describe a veraPDF pass only as conformance with its machine-verifiable PDF/UA-1 profile; it does not prove human checkpoints, reading order quality in practice, or assistive-technology interoperability.

Before describing the book as validated for beginners, run `book/references/beginner-validation-protocol.md` with unassisted novice readers and preserve scorer-produced aggregate and reader-app reports tied to exact committed artifacts, contract hashes, and an ordered attempt manifest. Use the preparation CLI to eliminate mechanical hash and ordering errors, not to generate answers or simulate readers. A self-reported manifest timestamp is not independent preregistration evidence; use an external append-only registry when that claim matters. Before making a comparative market claim, run a preregistered test against named alternatives. Internal editorial review is not independent expert endorsement.
