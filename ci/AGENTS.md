...
- `package.json` and `package-lock.json`: DAISY Ace CLI 1.4.6 and its integrity-locked dependency graph. Security overrides keep selected transitive parsers above current patched floors. `safe-extract-zip/` replaces the unpatched legacy ZIP fallback with a fail-closed rejection. The pinned Chrome archive is downloaded separately, checksum-verified, and expanded by macOS `ditto`; the untrusted EPUB stays on Ace's primary `node-stream-zip` path.
...
- Keep `test-safe-extract-zip.js` in the hosted gate. Any future extractor change must prove that the legacy fallback rejects input without writing outside its caller-managed destination.
