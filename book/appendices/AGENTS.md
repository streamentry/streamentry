...
- Appendix E is the one-page decision map. Keep the safety gate above the formal-practice and daily-life branches, keep the after-practice decision last, and preserve all four nodes as semantic text in EPUB. The grid itself is only a layout wrapper; each node is a named `group` bound by `aria-labelledby` to its visible question so it does not create a false note or landmark. It is an editorial retrieval aid bound to C74, not a canonical sequence or diagnostic flowchart. Its Vietnam 112–115 shorthand must remain linked to Chapter 9 and bounded by C75–C76.
