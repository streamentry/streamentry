...
| R09 | Pass | `book/theme.typ` uses semantic HTML elements and bookmarks rather than decorative-only structure; every final EPUB link has a non-empty accessible label, K01–K40 source links are individually named, and all 233 repeated titled cards resolve their visible title as a named `note` or `group` without polluting the heading or landmark outline. | None |
