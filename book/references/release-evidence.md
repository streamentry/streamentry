...
| PDF SHA-256 | `ebe9963d2721afb4bde149e6453c7596f58bab17a10eb893b2dc8be935ba79b6` |
| EPUB SHA-256 | `1da703b2615aae915fbc422e4bef835031b0f5c9f83c2929c82927ece6462866` |
| PDF extent | 190 A5 pages |
| PDF file size | 1,576,930 bytes |
| EPUB navigation | 196 nested content entries plus 1 cover entry |
| EPUB archive size | 194,595 bytes |
...
| Builder structural checks | Pass | Strict edition schema, XML, manifest, resources, contract-derived language and labels, accessibility metadata, navigation, every content-link label, all 206 local-fragment targets, all 56 absolute HTTPS source links, distinct labels for different external destinations, all 233 repeated-card visible-title bindings, bounded `note`/`group` roles, manuscript hash, required content, ZIP order, timestamp, and uncompressed mimetype. |
...
| Reader gateway | Pass internally | README presents the Vietnamese reader's PDF/EPUB choice, intended use, non-linear reading route, source model, safety boundary, correction path, candidate status and missing-rights warning before contributor build details. All relative links resolve to tracked files. It copies no artifact counts, does not call public access a license, and does not claim external validation or market leadership. |
| veraPDF 1.30.2 PDF/UA-1 | Pass internally | The checksum-pinned Greenfield release was forced to the `ua1` profile and returned a normal compliant result bound to the current PDF: 106 passed rules, 807,375 passed checks, 0 failed rules, and 0 failed checks. The fail-closed wrapper also requires one exact artifact, one exact version across the core, validation-model, and apps components, no parse/encryption/memory/exception failure, and a successful one-job batch. veraPDF documents this as machine-verifiable PDF/UA checking; it does not prove human checkpoints or assistive-technology interoperability. |
...
| Chromium accessibility tree | Pass internally | A full Chrome DevTools accessibility-tree read exposed 233 named repeated cards: 146 practice notes, 36 caution notes, 7 day groups, 40 reference groups, and 4 decision groups. Sample names matched the visible Vietnamese titles, including “Quy ước tối thiểu,” “Nếu chuyển động bụng không rõ,” and “Có việc phải bảo vệ ngay không?”. The result proves browser accessibility-tree naming only, not usability in a named screen reader or EPUB app. |
