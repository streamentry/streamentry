...
| Item | Evidence |
|---|---|
| Edition contract SHA-256 | `bd0db8768de441c8ada33da4a9ebb738d680b6ef20b27d939e89cefe7c8d3950` |
| Immutable source SHA-256 | `ad7a886895cf8cd29b369fda89de5665c96907d990f95dba8f028336bcbbd440` |
| PDF SHA-256 | `274687e2ce96b99ea1b8e255c5156e63a5a11b55928643ef781ed672fc020bdd` |
| EPUB SHA-256 | `b6b4df785cef997c306d58d4826ad6b650692c2808774df7306585b31d7ee570` |
| PDF extent | 243 A5 pages |
| PDF file size | 1,853,934 bytes |
| EPUB navigation | 278 nested content entries plus 1 cover entry |
| EPUB archive size | 251,714 bytes |
...
| Deterministic creation timestamp | `1787356800`, rendered as `2026-08-22T00:00:00Z` |
...
The builder acknowledged 201 allowlisted Typst HTML-export warnings and rejected unexpected warning classes. The print build names only the exact embedded families plus Inter, and release CI supplies the official Inter 4.0 files while disabling system fonts. Typst renders the EPUB cover directly instead of delegating rasterization to Poppler. A clean-checkout rebuild on the pinned macOS 15 ARM64 runner produced these bytes, and publication CI independently rebuilds and requires `git diff --exit-code` on both artifacts, so reproducibility is checked by the platform rather than assumed. The platform is part of the reproducibility contract; structurally valid builds on another operating system are not assumed to have the same hashes. Hosted CI obtains Poppler from Homebrew only for inspection, never artifact generation; the verifier fails closed unless the required stable fields and every page's geometry are present.
...
| veraPDF 1.30.2 PDF/UA-1 | Pass | The pinned hosted run returned 106 passed rules, 981,975 passed checks, 0 failed rules, and 0 failed checks under the forced `ua1` profile for the exact PDF bytes recorded above (run `35106058837`). This is machine evidence only; it does not cover the human checkpoints excluded from automated PDF/UA validation or interoperability with a named assistive-technology stack. |
| EPUBCheck 5.3.0 | Pass | The pinned hosted run returned 0 fatal errors, 0 errors, 0 warnings, and 0 informational messages for the exact EPUB bytes recorded above (run `35106058837`). |
| DAISY Ace 1.4.6 | Pass | The pinned hosted run inspected the exact EPUB bytes recorded above with Chrome for Testing 148.0.7778.97 and reported no issues in `nav.xhtml`, `cover.xhtml`, `book.xhtml` or `package.opf` (run `35106058837`). This is automated tooling, not a human reader-app smoke test. |
...
| PDF metadata and tagging | Pass | `pdfinfo` reports Vietnamese title metadata, the canonical credit, tagged structure, 243 unrotated A5 pages, no encryption, no JavaScript, and no suspects. Embedded text extraction preserves the first-sit and restart routes, source-code legend, source and safety boundaries, the three-level safety map, the first-three/five-lower/four-fruit distinctions, Chapter 10's canonical “who can attain?” cases, Chapter 12's beginner-first insight model with the four-region summary line, Chapter 13's four-task retrieval map, Chapter 7's pocket card and micro-practices, Appendix E's decision map, and the closing editorial-policy links. |
| PDF visual QA | Not rerun for these exact bytes | An earlier 243-page candidate was inspected page by page and at selected pages, but the wording edits that produced the current bytes can shift page positions, so that map is not transferred. No fresh full-page visual pass exists for the exact PDF recorded above; this does not substitute for an external print proof or assistive-technology test. |
