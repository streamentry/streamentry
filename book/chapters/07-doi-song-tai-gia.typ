#import "../components.typ": *

#chapter(
  [07],
  [Đem niệm vào một ngày sống],
  [Đời sống tại gia không đứng ngoài pháp hành. Chính lúc ăn, nói, làm việc, chăm người thân và chịu va chạm, ta thấy tâm đang nắm, đẩy hay sáng biết ra sao.],
  provenance: [
    #source-badge("KINH", refs: [K01; K15; K18; K19])
    #h(4pt)
    #source-badge("MAHĀSI", refs: [P01])
  ],
)

Kinh Niệm xứ đặt tỉnh giác giữa những việc rất thường: đi và về, nhìn thẳng và nhìn quanh, co và duỗi, ăn và uống, đại tiện và tiểu tiện, đi, đứng, ngồi, nằm, thức, nói và im lặng. Kinh không bảo ta phải biến cả ngày thành một chuỗi lời đọc thầm. Điều được đòi hỏi là biết rõ việc đang làm và trạng thái đang chi phối việc ấy.

#source-line("KINH", [K01], [MN 10 nêu rõ tỉnh giác trong các sinh hoạt của thân. Điệp khúc của bài kinh còn yêu cầu nhiệt tâm, tỉnh giác, chánh niệm, thấy sự sinh diệt và không nắm giữ. Chỉ gọi tên động tác chưa đủ thay cho những phẩm chất này.])

Vì thế, bốn điểm neo dưới đây cố ý ngắn và lặp lại. Mục tiêu không phải biến cả ngày thành một chuỗi ghi nhãn liên tục, mà là tạo vài chỗ chắc để người mới có thể quay về khi tâm trôi khỏi công việc đang làm.

== Ba cách dùng chú ý trong một ngày

Trong buổi ngồi, đối tượng chính có thể là chuyển động bụng. Ngoài đời, đối tượng phải nhường chỗ cho việc đang cần làm. Có lúc thực hành là biết thân và tâm; có lúc nó là nghe trọn một câu; có lúc nó chỉ là nhận ra mình đang mất chú ý rồi trả toàn bộ chú ý về đường đi, đứa trẻ hay công việc trước mặt.

#practice-card(
  [Chọn đúng chế độ, đừng cố làm cả ba],
  [
    + *Chú ý trọn việc:* khi lái xe, cầm dao, chăm người bệnh, đọc hợp đồng hoặc giải quyết một việc khó, phần lớn chú ý ở người và việc bên ngoài.
    + *Dừng ngắn để thấy phản ứng:* ở đoạn chuyển việc hoặc khi cảm xúc vừa dâng, nhận ra thân, thọ và ý muốn đang kéo mình. Chọn xong hành động thì trở lại hoàn cảnh.
    + *Thực hành có cấu trúc:* khi đã ở nơi an toàn và không có bổn phận cấp thời, mới dành riêng thời gian cho phồng xẹp, thiền hành hoặc quan sát chi tiết.

    Chánh niệm không được đo bằng lượng chú ý quay vào trong. Nó phải làm sự thấy biết hợp với việc cần làm ngay lúc ấy.
  ],
  label: [KHUNG SỬ DỤNG · BIÊN SOẠN THEO K01; K15; P01],
)

== Sống tỉnh giác không có nghĩa luôn làm thật chậm

Trong giờ thiền, đi chậm giúp nhận ra các pha của chuyển động. Ngoài đời, tốc độ phải hợp hoàn cảnh. Qua đường cần nhanh và nhìn xe. Cầm dao cần chắc. Bế con đang khóc cần đáp ứng ngay. Chánh niệm không làm ta chậm chạp; nó cắt bớt phần hấp tấp, lơ đãng và phản ứng thừa.

#modern-note([
  Chọn vài điểm neo có thể giữ lâu. Đừng cố ghi nhận từng chi tiết từ sáng đến tối. Sự cố sức quá mức dễ làm tâm căng, giảm khả năng làm việc và làm sinh hoạt của người thân bị ảnh hưởng không cần thiết.
])

#caution(
  [Khi sai một nhịp có thể gây tai nạn],
  [
    Lái xe, qua đường, dùng máy, bế trẻ gần nơi nguy hiểm hoặc xử lý một tình huống khẩn không phải lúc để nhắm mắt, theo dõi hơi thở hẹp hay tìm nhãn thật tinh. Giữ mắt mở, nhận biết toàn cảnh và làm điều an toàn cần làm. Nếu cần quan sát kỹ nội tâm, đợi đến khi người và việc đã an toàn. Đây là nguyên tắc an toàn hiện đại của người biên soạn, không phải câu chỉ dẫn nguyên văn trong MN 10.
  ],
)

== Bốn điểm neo đủ dùng <bon-diem-neo>

#source-line("BIÊN SOẠN", [bốn điểm neo, đối chiếu K01; P01], [
  Bốn điểm neo dưới đây là cách sách chọn vài sinh hoạt đã có trong Kinh Niệm xứ và chỉ dẫn Mahāsi rồi đặt vào một ngày sống hiện đại. K01 và P01 không ấn định đúng bốn thời điểm, số miếng ăn hay câu hỏi thầm này.
])

#practice-card(
  [Lúc vừa thức dậy],
  [Trước khi cầm điện thoại, biết thân đang nằm, biết ý muốn mở mắt và ý muốn ngồi dậy nếu chúng hiện rõ. Đừng dựng một ý định mà mình chưa kịp thấy. Sau đó nhắc lại năm giới bằng lời ngắn, rõ nghĩa.],
)

#v(8pt)

#practice-card(
  [Trong bữa ăn],
  [Dành ba miếng đầu để biết động tác chính, mùi vị và phản ứng thích hay không thích. Ăn với tốc độ bình thường. Khi nói chuyện cùng gia đình, sự hiện diện với người đối diện quan trọng hơn việc cố dán nhãn mọi lần nhai.],
)

#v(8pt)

#practice-card(
  [Ở những đoạn chuyển việc],
  [Khi đứng lên, mở máy, bước vào cuộc họp hoặc về đến cửa nhà, dừng một hơi thở. Hỏi thầm: “Tâm đang có gì?” Chỉ cần nhận ra trạng thái nổi bật như vội, bực, lo, ham muốn hay sáng rõ.],
)

#v(8pt)

#practice-card(
  [Trước khi nói],
  [Khi lời sắp bật ra trong lúc căng thẳng, cảm nhận thân trước: hàm có siết không, ngực có nóng không, hơi thở có gấp không. Một khoảng dừng ngắn có thể giữ lời nói khỏi thành vết thương.],
)

Bốn điểm neo chỉ có giá trị khi chúng đổi được lần phản ứng kế tiếp. Nơi dễ kiểm nhất không phải một buổi ngồi yên, mà là lúc người gần mình cần được lắng nghe hoặc đang làm ta khó chịu.

#practice-card(
  [Thẻ bỏ túi: bốn điểm neo trong một ngày],
  [
    #source-badge("BIÊN SOẠN")
    #v(5pt)

    + *Lúc thức dậy:* biết thân nằm, biết ý muốn ngồi dậy; nhắc năm giới bằng lời ngắn.
    + *Trong bữa ăn:* ba miếng đầu biết động tác, mùi vị và thích hay không thích.
    + *Khi chuyển việc:* một hơi thở; hỏi thầm “Tâm đang có gì?”.
    + *Trước khi phản hồi:* cảm nhận hàm, ngực, hơi thở trước khi lời bật ra.

    Thẻ này gom lại đúng bốn điểm neo vừa nêu để bạn tiện tra nhanh. Có thể chép ra giấy nhỏ hoặc lưu màn hình điện thoại; nó không thay thế phần giải thích đầy đủ ở trên và không phải thời khóa được kinh quy định.
  ],
  label: [THẺ TRA NHANH · BIÊN SOẠN],
)

== Một vòng ba mươi giây giữa va chạm <vong-va-cham>

Vòng #link(<tho-ai-nam-buoc>)[thọ đến ái ở chương 4] giúp thấy phản ứng từ lúc còn nhỏ. Nhưng giữa đời sống, không phải lúc nào cũng nên bắt đầu bằng nhìn vào trong. Nguy hiểm, bổn phận và nhu cầu của người đối diện có thể đi trước. “Ba mươi giây” chỉ là tên dễ nhớ; một tình huống nhanh có thể chỉ cho hai giây, một quyết định khó có thể cần lâu hơn.

#source-line("KINH", [K19 · AN 6.63], [
  AN 6.63 gọi tư, tức ý định, là nghiệp rồi nói đến hành động có chủ ý qua thân, lời và ý. Bài kinh giúp đặt ý định vào trung tâm của trách nhiệm. Nó không nói rằng người mới luôn kịp thấy rõ mọi ý định trước khi hành động.
])

#practice-card(
  [Năm việc, theo đúng thứ tự của hoàn cảnh],
  [
    + *Kiểm tra thực tại:* Có nguy hiểm, người cần bảo vệ hoặc việc phải làm ngay không? Nếu có, hành động trước; đừng dùng quan sát nội tâm để trì hoãn.
    + *Biết điều đang xảy ra rõ nhất:* hàm siết, ngực nóng, thọ khó chịu, tim nhanh. Đừng bắt đầu bằng bản cáo trạng về người kia.
    + *Nhận ra lực kéo:* muốn quát, muốn bỏ đi, muốn chứng minh, muốn mua, muốn mở thông báo. Nếu chỉ biết sau khi nó đã khởi, biết từ điểm ấy.
    + *Chọn phản ứng đơn giản nhất mà vẫn đúng hướng:* im một nhịp, hỏi lại, nói một ranh giới, hoãn trả lời, cất điện thoại hoặc làm việc an toàn trước mắt.
    + *Trở lại con người và nhiệm vụ:* nghe câu tiếp theo, hoàn tất việc đang làm. Đừng đứng mãi bên trong để chấm xem mình vừa chánh niệm đến đâu.
  ],
  label: [QUY TẮC TẠI CHỖ · BIÊN SOẠN THEO K01; K18; K19],
)

Bạn đang trả lời thư gấp thì thấy đứa trẻ chạy về phía cổng đang mở. Nóng mặt, muốn quát. Đưa trẻ khỏi chỗ nguy hiểm và đóng cổng trước, không tìm nhãn. Sau đó biết thọ khó chịu cùng ý muốn trách; nói ngắn, đúng sự việc. Nếu trẻ chỉ làm rơi một món đồ vô hại, khoảng dừng có thể đến sớm hơn. Cùng một phương pháp, thứ tự đổi theo hoàn cảnh.

== Gia đình là nơi thấy quả của sự tu tập

Khi con quấy, cha mẹ bệnh hoặc người bạn đời bất đồng, mục tiêu không phải làm cảm xúc biến mất. Trước hết, nhận ra sân, sợ, buồn hay ý muốn thắng. Sau đó mới chọn lời và hành động ít gây hại hơn. Chánh niệm đúng không khiến ta lạnh đi; nó giúp ta không giao tay lái cho phản ứng đầu tiên.

#source-line("KINH", [K18], [SN 55.7 trình bày việc tránh sát hại, lấy của không cho, tà hạnh và bốn loại lời nói có hại: dối trá, chia rẽ, thô ác, phù phiếm.])

#source-line("BIÊN SOẠN", [đối chiếu K18 với công thức Năm giới], [SN 55.7 liệt kê bảy điều tránh. Trong công thức Năm giới, giới thứ tư chỉ nêu tránh nói dối. So sánh này là phần đối chiếu của sách.])

#source-line("KINH", [K15 · DN 31], [
  DN 31 đặt đời sống cư sĩ trong những bổn phận qua lại đối với cha mẹ, thầy cô, vợ chồng và gia đình, bạn bè, người giao việc và người làm công, cùng người tu hành. Bài kinh còn bàn đến cách kiếm, giữ và sử dụng tài sản. Tu tại gia trong văn bản này không tách khỏi cách mình đối xử và làm tròn trách nhiệm với người khác.
])

Trong một cuộc tranh luận, “tôi đúng” có thể là một ý nghĩ đáng quan sát, nhưng không phải mọi bất đồng đều do ngã mạn. Có lúc ta cần nói rõ một ranh giới, ngăn một hành vi nguy hiểm hoặc sửa một thông tin sai. Chánh niệm phục vụ thấy biết và hành động đúng, không phục vụ sự im lặng dễ dãi.

#caution(
  [Chánh niệm không buộc bạn ở lại nơi nguy hiểm],
  [
    Nếu đang có đe dọa, cưỡng ép hoặc bạo lực, “ở lại quan sát” không phải nghĩa vụ. Tạo khoảng cách an toàn, gọi người có khả năng trợ giúp và dùng các nguồn hỗ trợ phù hợp với nơi mình sống. Biết sợ hoặc biết sân có thể giúp bớt phản ứng mù quáng; nó không biến hành vi nguy hiểm của người khác thành điều mình phải chịu. Đây là nguyên tắc an toàn hiện đại của người biên soạn.
  ],
)

== Khi khoảng dừng đến muộn: sửa thay vì tự kết án <sua-sai>

Người mới thường nhớ ra sau khi câu nặng lời đã bật khỏi miệng. Khoảnh khắc ấy vẫn dùng được. Chánh niệm không quay ngược thời gian, nhưng có thể chặn phần gây hại kế tiếp. Đừng dùng câu “tôi đang tu” để xin miễn trách nhiệm, cũng đừng biến một lỗi thành câu chuyện “tôi là người không thể tu”.

#source-line("BIÊN SOẠN", [vòng sửa sai, đối chiếu K15; K18], [
  K15 trình bày trách nhiệm trong các quan hệ và K18 nêu những hành vi cùng lời nói cần tránh. Năm bước dưới đây là quy trình biên soạn để đưa hai nguyên tắc ấy vào một lỗi vừa xảy ra. Đây không phải nghi thức sám hối nguyên văn từ hai bài kinh.
])

#practice-card(
  [Năm bước sau một phản ứng gây hại],
  [
    + *Dừng phần đang tiếp diễn:* hạ giọng, thôi nhắn thêm, đặt vật đang cầm xuống hoặc rời khỏi tình huống nếu an toàn đòi hỏi.
    + *Gọi đúng hành vi:* “Tôi vừa quát” rõ hơn “Tôi tệ quá”. Một câu nói về việc đã làm; câu kia khóa người làm vào một bản án.
    + *Nhận trách nhiệm không kèm chữ “nhưng”:* nghe tác động lên người kia trước khi giải thích ý tốt của mình.
    + *Sửa điều có thể sửa:* xin lỗi cụ thể, đính chính lời sai, đền hoặc thay món đồ mình đã làm hỏng, hoàn lại tiền hoặc thực hiện bổn phận đã bỏ.
    + *Chuẩn bị cách phòng lặp lại:* rời điện thoại khỏi bàn ăn, hoãn thư khi đang nóng, xin người khác tiếp quản vài phút hoặc dùng vòng va chạm ở đúng tín hiệu đầu tiên.

    Người bị hại không có nghĩa vụ tha thứ ngay. Sửa sai là phần trách nhiệm của người gây ra lỗi, không phải kỹ thuật buộc người khác làm mình nhẹ lòng.
  ],
  label: [PHỤC HỒI HÀNH VI · BIÊN SOẠN THEO K15; K18],
)

Cùng khoảng dừng trước lời nói ấy được mang sang công việc. Khác ở chỗ phần lớn chú ý lúc này phải phục vụ nhiệm vụ, không quay vào quan sát mình đến mức làm hỏng việc.

== Công việc cũng là nơi hành pháp

Khi làm việc trí óc, hãy để phần lớn chú ý ở nhiệm vụ. Dùng chánh niệm để nhận ra lúc tâm đã trôi, lúc thân căng quá mức hoặc lúc tham muốn hoàn thành khiến ta cẩu thả. Không cần chen một nhãn vào từng cú gõ phím.

Một cách thực tế là đặt ba lần dừng trong ngày: trước công việc khó nhất, trước bữa trưa và trước khi rời chỗ làm. Mỗi lần chỉ mất khoảng mười giây. Nếu thấy một việc cần xử lý, ghi nó ra rồi trở lại nhiệm vụ. Ghi nhận không thay thế lịch làm việc, quyết định hay đối thoại.

#practice-card(
  [Một lần bị gián đoạn ở bàn làm việc],
  [
    Thông báo hiện lên khi bạn đang đọc một đoạn khó. Mắt đã thấy; thọ dễ chịu hay khó chịu có thể sinh; ý muốn mở tin nhắn kéo tay đi. Nếu tin có thể chờ, biết lực kéo, ghi việc cần nhớ vào một chỗ đáng tin rồi trở lại đúng dòng đang đọc. Nếu đó là cảnh báo an toàn, việc khẩn hoặc trách nhiệm phải trả lời, xử lý nó rồi chủ động quay lại.

    Mục tiêu không phải chưa từng bị gián đoạn. Mục tiêu là biết mình đã rời việc nào, vì sao phải rời, và có một đường quay về.
  ],
  label: [TÌNH HUỐNG LÀM VIỆC · BIÊN SOẠN],
)

== Bài tập vi mô cho ngày quá bận

Ngày nào cũng có những khe hở ngắn không cần lịch, không cần chỗ ngồi và không ai nhận ra. Vài bài dưới đây chỉ mất từ ba mươi giây đến hai phút; chúng là cách biên soạn giúp nuôi tính liên tục giữa việc, không phải bổn phận hay thước đo tiến bộ.

#practice-card(
  [Bốn bài tập vi mô dễ nhớ],
  [
    #source-badge("BIÊN SOẠN")
    #v(5pt)

    + *Ba mươi giây trước khi gõ phím:* dừng lại một nhịp trước khi trả lời một tin nhắn gây tranh cãi; cảm nhận hơi thở vào ra một lần.
    + *Ba hơi thở khi đèn đỏ:* chân chạm sàn xe, vai thả lỏng, biết đang ngồi; đèn xanh thì tiếp tục lái.
    + *Một phút ở bàn chân:* bước ra khỏi thang máy hoặc cửa công ty, đứng lại cảm nhận bàn chân tiếp đất trong vài bước đầu.
    + *Ba miếng ăn đầu tiên:* trong bữa cơm gia đình, để điện thoại xa tay và biết mùi vị cùng người đối diện.

    Nếu ngày quá căng, một bài duy nhất đã đủ. Đừng bù bài bị lỡ; chọn lại bài dễ làm nhất vào hôm sau.
  ],
  label: [VI MÔ · BIÊN SOẠN],
)

== Lo âu về tài chính và việc bất ổn

Nỗi lo tiền bạc, bảo hiểm hay công việc có thể kéo dài cả ngày và dễ bị gọi là “thiếu buông bỏ”. Thực tế, nhiều lo âu như vậy bắt nguồn từ trách nhiệm thật, cần được giải quyết bằng kế hoạch chứ không phải bằng cố nhìn nội tâm thêm. Chánh niệm ở đây có ba việc nhỏ: biết thọ khó chịu đang có mặt thay vì cãi nó; biết lực kéo sang tìm tin xấu hoặc né tránh nhìn hóa đơn; rồi quay về một hành động cụ thể nằm trong tay — ghi rõ khoản cần kiểm tra, đặt lịch hỏi bảo hiểm, hoặc nói với người thân một kế hoạch. Việc hành chính vẫn là việc hành chính; chánh niệm chỉ giúp phản ứng không kéo tình hình đi xa hơn.

#practice-card(
  [Khi nỗi lo tài chính nổi lên],
  [
    #source-badge("BIÊN SOẠN")
    #v(5pt)

    + *Đặt tên ngắn:* “đang lo”, “đang sợ thiếu”.
    + *Hỏi một câu:* việc nhỏ nhất nằm trong tay mình hôm nay là gì?
    + *Làm đúng việc ấy* hoặc ghi vào danh sách nếu chưa thể làm ngay.
    + *Nếu lo âu làm mất ngủ, mất ăn hoặc kéo dài hàng tuần:* đó là lúc dùng #link(<an-toan>)[các mức phản ứng ở chương 9] và tìm hỗ trợ sức khỏe tâm thần; đó không phải thất bại trong tu tập.
  ],
  label: [QUYẾT ĐỊNH THỰC · BIÊN SOẠN],
)

== Cuối ngày: nhìn lại hành vi, không chấm tầng tuệ

#practice-card(
  [Năm câu hỏi trước khi ngủ],
  [
    #source-badge("BIÊN SOẠN")
    #v(5pt)

    - Hôm nay giới nào được giữ vững, giới nào sứt mẻ?
    - Tôi đã nhận ra một cảm xúc trước khi nó thành lời hay hành động chưa?
    - Lúc nào tôi biết mình đang phóng tâm và trở lại được?
    - Có ai phải chịu thêm khó khăn vì cách tôi thực hành không?
    - Ngày mai tôi sẽ giữ một điểm neo nào?
  ],
  label: [TỰ KIỂM · BIÊN SOẠN],
)

Đứt nhịp vài ngày? Mở #link(<khoi-dong-lai>)[lối khởi động lại ở chương 1]; đừng tự đặt lịch bù.

#v(8pt)

#source-line("KINH", [K14], [AN 6.19 dạy niệm chết với mức khẩn thiết rất cao, tính đến một miếng ăn hay một hơi thở. Việc rà ngày hai phút là cách biên soạn hiện đại để nuôi tinh tấn; đó không phải thời khóa được bài kinh quy định.])

== Thước đo khiêm tốn

Có thể theo dõi số ngày đã hành, mức độ giữ giới, tốc độ nhận ra phóng tâm và khả năng dừng trước một phản ứng gây hại. Nhận xét của người thân cũng đáng nghe. Những dấu hiệu này cho biết thói quen và hành vi đang đổi ra sao; chúng không chứng nhận một tầng tuệ hay thánh quả.

#caution(
  [Đừng biến người thân thành giám khảo],
  [
    Người thân có thể thấy những điều ta bỏ sót, nhưng họ không phải dụng cụ đo giác ngộ. Hãy hỏi về hành vi cụ thể: mình có bớt quát, biết lắng nghe và giữ lời hơn không? Đừng hỏi họ xác nhận một danh hiệu tâm linh.
  ],
)

== Gấp sách lại: bốn việc phải tự làm được

#practice-card(
  [Kiểm bằng tình huống, không kiểm bằng khẩu hiệu],
  [
    + Khi công việc có rủi ro, tôi biết đặt chú ý vào an toàn thay vì cố quan sát nội tâm.
    + Khi phản ứng vừa dâng, tôi nhận ra điều đang xảy ra ở thân, thọ và lực kéo, rồi chọn một hành động hợp hoàn cảnh.
    + Khi đã lỡ gây hại, tôi biết dừng, nhận đúng việc, sửa cụ thể và chuẩn bị cách phòng lặp lại.
    + Khi nhịp thực hành gãy, tôi quay về lối khởi động lại thay vì bù giờ hoặc bỏ luôn.

    Nếu chưa làm được câu hai, đọc lại #link(<vong-va-cham>)[vòng va chạm]. Nếu chưa làm được câu ba, đọc lại #link(<sua-sai>)[vòng sửa sai]. Hai vòng này đo bằng hành vi kế tiếp, không đo bằng cảm giác mình là người tu tốt.
  ],
  label: [KIỂM TRA KHẢ NĂNG LÀM · BIÊN SOẠN],
)
