...
Chapters carry the explanatory arc from a seven-day start through the full path, dependent origination at feeling and craving, Mahāsi technique, daily life, safety, a focused explanation of the first three fetters, a separate five-lower-fetters and four-fruits map, and the insight map as later reference. Prefer files below 300 lines, but do not compress sourced explanation merely to satisfy the line target.
...
```mermaid
flowchart LR
  A["Orientation"] --> B["Foundations"] --> C["Dependent origination"] --> D["Technique"] --> E["Integration"] --> F["Safety"] --> G["First three fetters"] --> H["Five lower fetters and four fruits"] --> I["Insight map as later reference"]
```
