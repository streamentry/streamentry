...
- `external_review_packet_content.py`: canonical gate order, Vietnamese assignment text and guide, manifest, immutable packet data structures, and the complete beginner-pilot runtime path set. Every assignment must name three ordered startup actions and its exact evidence-role output. Participant-material warnings belong only to participant gates. The packet must let a coordinator run the frozen preparation CLI and scorer without fetching code from the network, while explicitly withholding the packet, rubric, schemas and scorer from pilot readers.
